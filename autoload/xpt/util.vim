...
fun! xpt#util#DeepExtend(to,from)
	for key in keys(a:from)
		if type(a:from[key]) == type({})
			if has_key(a:to,key)
				if a:from[key] == {} && a:to[key] != {}
					continue
				endif
		...
		elseif type(a:from[key]) == type([])
			if has_key(a:to,key)
				if a:from[key] == [] && a:to[key] != []
					continue
				endif
				if key == "comeLast"
					let a:to.comeFirst = xpt#util#RemoveDuplicateFrom(a:to.comeFirst, a:from[key])
					let pos = index(a:to[key], "cursor")
					if pos == -1
						call extend(a:to[key],a:from[key])
					else
						call extend(a:to[key],a:from[key], pos)
					endif
					let a:to[key] = xpt#util#RemoveDuplicate(a:to[key])
				elseif key == "comeFirst"
					let a:to.comeLast = xpt#util#RemoveDuplicateFrom(a:to.comeLast, a:from[key])
					call extend(a:to[key],a:from[key], 0)
					let a:to[key] = xpt#util#RemoveDuplicate(a:to[key])
				else
					call extend(a:to[key],a:from[key])
					"let a:to[key] = a:from[key]
...
fun! xpt#util#RemoveDuplicateFrom(list, list2)
	let newList = []
	for e in a:list
		if index(a:list2, e) == -1
			call add(newList,e)
		endif
	endfor
	return newList
