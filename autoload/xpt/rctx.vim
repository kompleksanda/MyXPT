...
let p = s:phase
let s:phaseGraph = { p.uninit : [p.popup,p.inited], p.popup : [p.inited], p.inited : [p.rendering], p.rendering : [p.rendered], p.rendered : [p.iteminit], p.iteminit : [p.iteminit,p.fillin,p.post], p.fillin : [p.post,p.finished,p.iteminit], p.post : [p.finished,p.iteminit, p.popup], p.finished : [p.uninit], }
unlet p
fun! xpt#rctx#New(x)
