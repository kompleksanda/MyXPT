...
fun! XPTemplateSetting(name,toWhich,setting, snip)
	let name = a:name
	let xptObj = b:xptemplateData
	let ftScope = xptObj.filetypes[g:GetSnipFileFT()]
	let xt = ftScope.allTemplates
	let prio = xptObj.snipFileScope.priority
	let existed = get( xt, a:name, { 'priority': xpt#priority#Get( 'lowest' ) } )
	if existed.priority < prio
		return
	endif
	let supers = split(a:toWhich, '|')
	let setting = {}
	for super in supers
	...
	call xpt#util#DeepExtend(setting,a:setting)
	call g:XPTapplyTemplateSettingDefaultValue(setting)
	if type(a:snip) == type([])
...
fun! s:DoInclude(tmplDict,tmplObject,pattern,keepCursor)
	...
	while 1
		...
		if len(incName) >= 3 && incName[-2:] == "!!"
			let toForce = 1
			let incName = incName[:-3]
		else
			let toForce = 0
		endif
...
fun! s:MergeSetting(toSettings,fromSettings)
	...
	call extend( a:toSettings.ontypeFilters, a:fromSettings.ontypeFilters, 'keep' )
...
fun! g:MyMergeSetting(toSettings,fromSettings)
	"let a:toSettings.comeLast = xpt#util#RemoveDuplicateFrom(a:toSettings.comeLast, a:fromSettings.comeFirst)
	"let a:toSettings.comeFirst += a:fromSettings.comeFirst
	"let a:toSettings.comeFirst = xpt#util#RemoveDuplicateFrom(a:toSettings.comeFirst, a:fromSettings.comeLast)
	"let a:toSettings.comeLast = a:fromSettings.comeLast + a:toSettings.comeLast
	"let a:toSettings.comeLast = xpt#util#RemoveDuplicate(a:toSettings.comeLast)
	"let a:toSettings.comeFirst = xpt#util#RemoveDuplicate(a:toSettings.comeFirst)
	"let pos = index(a:toSettings.comeLast, "cursor")
	"if pos != -1
	"	call remove(a:toSettings.comeLast, pos)
	"endif
	"call add(a:toSettings.comeLast, "cursor")
	call s:InitItemOrderList(a:toSettings)
	call extend( a:toSettings.preValues, a:fromSettings.preValues, 'keep' )
	call extend( a:toSettings.defaultValues, a:fromSettings.defaultValues, 'keep' )
	call extend( a:toSettings.postFilters, a:fromSettings.postFilters, 'keep' )
...
fun! s:MergeSettingForce(toSettings,fromSettings)
	call xpt#util#DeepExtend(a:toSettings, a:fromSettings)
endfunction
...
fun! s:NewRenderContext(ftScope,tmplName)
	...
	"echom "New renderContext created for " . a:tmplName
...
fun! g:MyRunDuringRendering(ftScope)
	let so = b:xptemplateData.renderContext.snipObject
	for [keyy, vall] in items(so.setting.variables)
		if type(vall) == type('') && vall =~ '\V\^->runr<-'
			silent! let so = deepcopy(b:xptemplateData.renderContext.snipObject)
			let spro = {"name":so.name,"snipText":so.snipText,"setting":so.setting}
			let vall = xpt#util#UnescapeChar(vall[8:], '($')
			let spro.setting.variables[keyy] = xpt#eval#Exec(vall, [a:ftScope.funcs, so.setting.variables, spro])
			let so.name = spro.name
			let so.snipText = spro.snipText
			let so.setting = spro.setting
			let b:xptemplateData.renderContext.snipObject = so
		endif
	endfor
endfunction
fun! s:RenderSnippet()
	...
	call g:MyRunDuringRendering(sess.data.ftScope)
...
fun! g:MyMergeSSetting(thisSetting, query )
	let xptObj = b:xptemplateData
	let xt = xptObj.filetypes[g:GetSnipFileFT()].allTemplates
	...
	for [super, sett] in items(a:query) " {super:sett, ...}
		...
		if toSnip is 0
			if super == "" || super == g:_MyCurParseSnip.name
				let toSnip = {"setting":a:thisSetting}
			else
				continue
			endif
		endif
		if type(sett) != type({})
			...
			for namee in sett " {super : [namee, ...]}
				...
				if namee =~ '\V\^$\w\+\$'
					...
					let total = ["preValues","ontypeFilters","defaultValues"]
					for name in total
			...
			for [namee, listt] in items(sett) "{super:{namee:listt, ...}, ...} 
				...
				let curr = deepcopy(toSnip.setting)[namee]
				let thiscurr = setting[namee]
				if namee == "comeLast"
					if currname != ""
						let setting.variables[currname] = curr
						continue
					endif
					let setting.comeFirst = xpt#util#RemoveDuplicateFrom(setting.comeFirst, curr)
					let pos = index(thiscurr, "cursor")
					if pos == -1
						call extend(thiscurr,curr)
					else
						call extend(thiscurr,curr, pos)
					endif
					let thiscurr = xpt#util#RemoveDuplicate(thiscurr)
				elseif namee == "comeFirst"
					if currname != ""
						let setting.variables[currname] = curr
						continue
					endif
					let setting.comeLast = xpt#util#RemoveDuplicateFrom(setting.comeLast, curr)
					call extend(thiscurr,curr, 0)
					let thiscurr = xpt#util#RemoveDuplicate(thiscurr)
				elseif type(listt) == type(0)
					if currname != ""
						let setting.variables[currname] = curr
					else
						call xpt#util#DeepExtend(thiscurr, curr)
	...
	call xpt#util#DeepExtend(a:thisSetting, setting)
endfunction
fun! g:MyParseRepetition(snipObject, postFilters, snip)
	let tmplObj = a:snipObject
	let xp = a:snipObject.ptn
	let postFilters = a:postFilters
	let tmpl = a:snip
	let bef = ""
	let rest = ""
	let rp = xp.lft . s:repetitionPattern . xp.rt
	...
	while stack != []
		...
		let symbol = matchstr(rest,rp)
		let name = substitute( symbol, '\V' . xp.lft . '\|' . xp.rt, '', 'g' )
		let postFilters[name] = xpt#flt#New(-indentNr,repeatPart)
		let bef .= symbol
		let rest = substitute(rest, repPtn, '', '')
		let tmpl = bef . rest
	endwhile
...
fun! g:MyParseQuotedPostFilter(tmplObj, postFilters, quoter, snip)
	let xp = a:tmplObj.ptn
	let postFilters = a:postFilters
	let quoter = a:quoter
	let flagPattern = '\V\[!]\$'
...
fun! s:BuildSnippet(nameStartPosition,nameEndPosition)
	...
	"echom "Building snippet " . ctx.snipObject.name . "--------------"
	...
	"echom "Snippet " . ctx.snipObject.name . " built --------------"
	"echom "Move down the previous renderContext if avaible" .  empty(x.stack)
...
fun! PhsGetNameInfo(end)
	return s:GetNameInfo(a:end)
endfunction
fun! PhsGetValueInfo(end)
	return s:GetValueInfo(a:end)
endfunction
...
fun! s:CreatePlaceHolder(ctx,nameInfo,valueInfo)
	...
	"echom "creating placeHolder " . name
...
fun! s:BuildMarksOfPlaceHolder(item,placeHolder,nameInfo,valueInfo)
	"echom "     Building Marks of placeHolder " . a:placeHolder.name . " in item " . a:item.name
	let renderContext = b:xptemplateData.renderContext
	...
	"echom "             placeHolder has been DISPLAYED On screen"
	...
	"echom "              marks added to b:_xpmark " . placeHolder.mark.start
endfunction
fun! s:AddItemToRenderContext(ctx,item)
	"echom "      adding item to renderContext, phase=" . a:ctx.phase
	...
	if exist
		"echom "             item already added to renderContext(ctx.itemDict[item]"
		return
	endif
	if ctx.phase != 'rendering'
		if ! s:AddToOrderList(ctx.firstList,item)
			...
			"echom ctx.action . "==============="
			"echom "             item added to END of " . ctx.item.name . '.firstList'
		endif
		call filter( ctx.itemList, 'v:val isnot item' )
		"echom "             item removed from " . ctx.item.name . '.itemList if it exists'
		return
	endif
	if item.name == ''
		call add(ctx.itemList,item)
	elseif s:AddToOrderList(ctx.firstList,item) || s:AddToOrderList(ctx.lastList,item)
		"echom "             item already in firstList or lastList"
		return
	else
		call add(ctx.itemList,item)
	endif
	"echom "             item added to END of itemList"
endfunction
...
fun! s:BuildPlaceHolders(markRange)
	...
	let x = b:xptemplateData
	let renderContext = b:xptemplateData.renderContext
	"echom "Building all placeHolders in snippet " . renderContext.snipObject.name
	...
	endwhile
	"echom "All placeHolders in snippet " . renderContext.snipObject.name . " built"
...
fun! s:BuildPlaceHolders2(markRange)
	let s:buildingSeqNr += 1
	let rc = 0
	let x = b:xptemplateData
	let renderContext = b:xptemplateData.renderContext
	...
	"echom "All placeHolders in snippet " . renderContext.snipObject.name . " built"
	call filter( renderContext.itemList, 'type(v:val) != 1' )
	call filter( renderContext.firstList, 'type(v:val) != 1' )
	call filter( renderContext.lastList, 'type(v:val) != 1' )
	let end = XPMpos(a:markRange.end)
	call cursor(end)
	let [renderContext.item,renderContext.leadingPlaceHolder] = current
	let renderContext.action = ''
...
fun! s:EvaluateEdge(xp,item,ph)
	if !a:ph.isKey
		if !has_key(a:ph, 'leftEdge') && !has_key(a:ph, 'rightEdge')
			return
		endif
		let mmark = a:ph.innerMarks
	else
		let mmark = a:ph.editMark
	endif
	let eval_ptn = '\V' . a:xp.item_var . '\|' . a:xp.item_func
	if a:ph.leftEdge =~ eval_ptn
		let a:ph.leftEdge = s:EvalAsFilter(a:ph.leftEdge, XPMpos(a:ph.mark.start))
		call XPreplaceByMarkInternal(a:ph.mark.start,mmark.start, a:ph.leftEdge)
	endif
	if a:ph.rightEdge =~ eval_ptn
		let a:ph.rightEdge = s:EvalAsFilter(a:ph.rightEdge, XPMpos(mmark.end))
		call XPreplaceByMarkInternal(mmark.end,a:ph.mark.end, a:ph.rightEdge)
	endif
	let a:ph.fullname = a:ph.leftEdge . a:item.name . a:ph.rightEdge
	let a:item.fullname = a:ph.fullname
endfunction
...
fun! s:ApplyBuildTimeInclusion(placeHolder,nameInfo,valueInfo)
	...
	if toForce
		silent! let dcopy = deepcopy(renderContext.snipObject)
		call s:MergeSettingForce(dcopy.setting,incTmplObject.setting)
		let renderContext.snipObject = dcopy
		let renderContext.snipSetting = copy(renderContext.snipObject.setting)
	else
...
fun! s:BuildItemForPlaceHolder(placeHolder)
	let renderContext = b:xptemplateData.renderContext
	"echom "Building item for placeHolder:" . a:placeHolder.name . " phase=" . renderContext.phase
	if has_key(renderContext.itemDict,a:placeHolder.name)
		let item = renderContext.itemDict[a:placeHolder.name]
		"echom "     but a placeHolder with this name already exists in renderContext ctx.itemDict[name]"
	else
	...
	if a:placeHolder.isKey
		...
		if renderContext.phase != 'rendering' && inPrevBuild
			...
			"echom "       added this placeHolder IN FRONT of " . item.name . ".placeHolders"
		else
			call add(item.placeHolders,a:placeHolder)
			"echom "       added this placeHolder at THE END of " .  item.name . ".placeHolders"
		endif
	endif
	return item
endfunction
...
fun! XPTforceForward(action)
	...
	"echom "-----XPTforceForward called-" . a:action
...
fun! s:FinishCurrent(action)
	...
	if (built && built != 2) || a:action ==# 'clear'
	...
	if filter isnot g:EmptyFilter
		...
		if flt_rst.rc != 0 && typed != ""
			...
			if has_key(flt_rst, 'action') && flt_rst.action ==# 'pum'
				...
				"call feedkeys(s:DefaultValuePumHandler(rctx, flt_rst), 'n')
			elseif has_key(flt_rst, 'action') && flt_rst.action ==# 'expandTmpl'
...
fun! s:DoGotoNextItem()
	let renderContext = b:xptemplateData.renderContext
	"echom "(Extract) Going to next item"
	...
	let renderContext = b:xptemplateData.renderContext
	let leader = renderContext.leadingPlaceHolder
	if placeHolder == s:nullDict && renderContext.processing && empty(renderContext.itemList) && !has_key(renderContext.snipSetting.postFilters,renderContext.item.name) && !has_key( leader, 'postFilter' ) && empty(renderContext.item.placeHolders) && XPMpos(leader.mark.end) == XPMpos(renderContext.marks.tmpl.end) && postaction !~ ''
...
fun! s:HandleDefaultValueAction(flt_rst)
	...
	elseif a:flt_rst.nav == "cancel"
		if x.renderContext.processing
			let postaction =  s:ShiftForward('clear')
			return postaction
		else
			return ''
		endif
...
fun! s:LoadFilterActionSnippet(flt_rst)
	...
	if has_key( a:flt_rst, 'snippet' )
		...
		if len(snipname) >= 3 && snipname[-2:] == "!!"
			let toForce = 1
			let snipname = snipname[:-3]
		else
			let toForce = 0
		endif
		if has_key(allsnip,snipname)
			...
			if toForce
				silent! let dcopy = deepcopy(renderContext.snipObject)
				call s:MergeSettingForce(dcopy.setting,snip.setting)
				let renderContext.snipObject = dcopy
				let renderContext.snipSetting = copy(renderContext.snipObject.setting)
			else
...
fun! s:oldUpdateFollowingPlaceHoldersWith(contentTyped,option)
	...
	"let current1 = [renderContext.item,renderContext.leadingPlaceHolder, b:_xpmark]
	...
	for ph in phList
		...
		"echom "updating " . "...." . renderContext.phase . "......... " . renderContext.leadingPlaceHolder.mark.start
		let flt = renderContext.phase == 'post' ? get( ph, 'postFilter', get( ph, 'ontimeFilter',  g:EmptyFilter ) ) : get( ph, 'ontimeFilter', g:EmptyFilter )
		let [start, end] = XPMposStartEnd(ph.mark)
		let [phln, phcol] = start
		if xpt#util#TextBetween([start, end]) !=# a:contentTyped
			"let relPos = s:RecordRelativePosToMark( [ line( '.' ), col( '.' ) ], current1[1].mark.start )
			"call XPreplace(start,end, a:contentTyped)
			"renderContext.marks.tmpl.end
			"call s:GotoRelativePosToMark(relPos, current1[1].mark.start)
		endif
		...
		call s:MyOwnHandler(flt_rst, ph, a:contentTyped)
		"let postaction = s:MyOwnHandler(flt_rst, ph, a:contentTyped)
		"if '' != postaction
		"	echom "po[[]][[]]]]------" . postaction
	...
	let renderContext.firstList = []
	let renderContext.lastList = []
	"let [renderContext.item,renderContext.leadingPlaceHolder, b:_xpmark] = current1
...
fun! s:MyOwnHandler(flt_rst, ph) "{{{
	...
	let ph = a:ph
	let mark_name = s:GetPHReplacingMarkName(flt_rst)
	let marks = ph[mark_name]
	let [start,end] = XPMposStartEnd(marks)
	if flt_rst.rc != 0
		if has_key(flt_rst,'action') &&  flt_rst.action ==# 'expandTmpl'
			call XPreplace(start, end, '')
			call XPMsetLikelyBetween(marks.start,marks.end)
			call XPTemplateStart(0, {'startPos' : getpos(".")[1:2], 'tmplName' : flt_rst.tmplName})
		elseif has_key(flt_rst, 'action') && flt_rst.action ==# 'pum'
			call XPreplace(start,end,tlib#input#List("s", " ". ph.name. "->" .marks.start, flt_rst.pum))
			"return s:DefaultValuePumHandler(renderContext,a:flt_rst)
		...
		elseif has_key( flt_rst, 'text' )
			...
			if has_key(flt_rst,'action') && flt_rst.action ==# 'build'
				...
				if renderContext.item.name != ''
					let renderContext.namedStep[renderContext.item.name] = text
				endif
				let buildrc = s:BuildPlaceHolders2(marks)
				if 0 > buildrc
					call s:Crash()
				endif
			endif
		endif
		"if has_key(flt_rst,'nav') && flt_rst.nav == 'next' && ((has_key(flt_rst, 'action') && index(['build','finishTemplate','expandTmpl','pum'], flt_rst.action) == -1) || (!has_key(flt_rst, 'action')))
		"	if renderContext.processing
		"		call s:ShiftForward( '' )
		"	endif
		"elseif has_key(flt_rst,'nav') && flt_rst.nav == "cancel"
		"	if renderContext.processing
		"		call s:ShiftForward('clear')
		"	endif
		"endif
	endif
	return ""
...
fun! s:XPTupdateTyping()
	...
	let flt = get(leader, "ontimeFilter", get(ontypeFilters,leader.name, g:EmptyFilter))
	if flt isnot g:EmptyFilter
...
fun! s:DoUpdate(renderContext,changeType)
	...
	if type(a:changeType) == type([]) || a:changeType is g:XPM_RET.likely_matched || a:changeType is g:XPM_RET.no_updated_made
		...
		"echom "UPDATING placeHolders of " . renderContext.item.name . ", with value:" . contentTyped . ", mark of leadingPlaceHolder is " . renderContext.leadingPlaceHolder.mark.start
		call s:UpdateFollowingPlaceHoldersWith(contentTyped,{})
		"echom "UPDATED " . "...................... " . renderContext.item.name . ", " . renderContext.leadingPlaceHolder.mark.start
