...
fun! xpt#parser#CompileSnippet(lines)
	...
	let g:_MyCurParseSnip = {"name":snippetName}
	...
	while start < len
		...
		if command != ''
			let [key,val,start] = s:GetXSETkeyAndValue(lines,start, setting)
			if key == ''
				let start += 1
				continue
			endif
	...
	for [keyy, vall] in items(setting.variables)
		if type(vall) == type('')
			if vall =~ '\V\^->runp<-'
				...
				let spro = {"name":snippetName,"snipText":snippetLines,"setting":setting}
				let vall = xpt#eval#Exec(vall[8:], [b:xptemplateData.filetypes[b:xptemplateData.snipFileScope.filetype].funcs, setting.variables, spro])
				let snippetName = spro.name
				let snippetLines = spro.snipText
				let setting = spro.setting

			endif
			if type(vall) == type('')
				let vall = g:MyParseRepetition(b:xptemplateData.snipFileScope, setting.postFilters, vall)
				let setting.variables[keyy] = g:MyParseQuotedPostFilter(b:xptemplateData.snipFileScope, setting.postFilters, { 'end':'}}', 'start':'{{' }, vall)
			else
				let setting.variables[keyy] = vall
			endif
		endif
	endfor
	...
	elseif has_key(setting, 'setting')
		return printf('call XPTemplateSetting(%s,%s,%s,%s)',string(snippetName),string(setting.setting), string(setting), string(snippetLines))
	else
...
fun! s:HandleXSETcommand(setting,command,cmdArgs)
	...
	elseif kn =~ '\V\^$'
		if val =~ '\V\^->keep<-'
			let a:setting.variables[kn] = val[8:]
		elseif val =~ '\V\^->runp<-' || val =~ '\V\^->runr<-'
			let a:setting.variables[kn] = val
		else
			let a:setting.variables[kn] = xpt#eval#Eval(val, [b:xptemplateData.filetypes[b:xptemplateData.snipFileScope.filetype].funcs, a:setting.variables])
		endif
		let valuee = a:setting.variables[a:keyname]
		if a:keyname == "$_import_" && valuee[0:0] == "{"
			call g:MyMergeSSetting(a:setting, json_decode(valuee))
			return
		elseif a:keyname == "$_preValues_"
			for [n,v] in items(json_decode(valuee))
				for n in split(n, "|")
					if type(v) == type(0)
						let a:setting.preValues[n] = xpt#flt#New(0, n)
					else
						let a:setting.preValues[n] = xpt#flt#New(0, v)
					endif
				endfor
			endfor
		elseif a:keyname == "$_postValues_"
			for [n,v] in items(json_decode(valuee))
				for n in split(n,"|")
					if type(v) == type(0)
						if n =~ '\V...'
							let a:setting.postFilters[n] = xpt#flt#New( 0, 'BuildIfNoChange(Embed(V()))' )
						else
							let a:setting.postFilters[a:keyname] = xpt#flt#New(0,'Embed(V())')
						endif
					else
						if n =~ '\V...'
							let a:setting.postFilters[n] = xpt#flt#New( 0, 'BuildIfNoChange(' . string(v) . ')' )
						else
							let a:setting.postFilters[n] = xpt#flt#New(0,v)
						endif
					endif
				endfor
			endfor
...
fun! xpt#parser#ParseSnippet(p)
	...
	if s != -1
		...
		let insideL += range(s, min([blk,i]))
	endif
	"let newLines = []
	"let i = 0
	"while i < len -1 | let i += 1
	"	if index(insideL, i) == -1 && lines[i] !~# '^\.\.XPT' && lines[i] != ""
	"		call add(newLines, lines[i])
	"	endif
	"endwhile
	"let newLines = join(newLines, "|")
	"if newLines != ""
	"	execute newLines
	"endif
	call xpt#snipfile#Pop()
...
fun! s:XPTemplateParseSnippet(lines)
	...
	elseif has_key(setting, 'setting')
		call XPTemplateSetting(snippetName,setting.setting, setting, snippetLines)
	else
...
fun! s:GetXSETkeyAndValue(lines,start, setting)
	...
	if isMultiLine
		let key = XSETparam
		let [start,val] = s:ParseMultiLineValues(a:lines,start, key)
		if key[0:0] == "$"
			if stridx(key,"+") != -1
				let sp = split(key, "+")
				let key = sp[0]
				if len(sp) == 1 || (len(sp) == 2 && sp[1] == "")
					let va = a:setting.variables[key]
				elseif len(sp) == 3 && sp[1] == ""
					let va = a:setting.variables[sp[2]]
				else
					let va = a:setting.variables[sp[1]]
				endif
				if va[:7] == "->runp<-"
					if (len(sp)==3 || len(sp)==2) && sp[1] == ""
						let val = "->runp<-" . val . va[8:]
					else
						let val = "->runp<-" . va[8:] . val
					endif
				elseif va[:7] == "->runr<-"
					if (len(sp)==3 || len(sp)==2) && sp[1] == ""
						let val = "->runr<-" . val . va[8:]
					else
						let val = "->runr<-" . va[8:] . val
					endif
				else
					if (len(sp)==3 || len(sp)==2) && sp[1] == ""
						let val = val . va
		...
		if key[-1:-1] == "\\"
			let rkey = key
			while key[-1:-1] == "\\"
				let mres = "=" . matchstr(XSETparam[len(key)+1:],  '\V\[^=]\*\ze=')
				let rkey = rkey[:-2] . mres
				let key .= "=" . mres
			endwhile
			let key = rkey
		endif
		let val = matchstr(XSETparam, '\V'.s:nonEscaped.'=\s\*\zs\.\*')
		let val = substitute(val, '\\n', "\n", 'g')
		if key[-1:-1] ==  '+' && key[0:0] == "$"
			let key = key[:-2]
			if key == '$'
...
fun! s:ParseMultiLineValues(lines,start, key)
	let lines = a:lines
	let start = a:start
	...
	let val = xpt#util#UnescapeChar(val,'X')
	return [start,val]
endfunction
fun! s:HandleXSETcommandOld(setting,command,keyname,keytype,value)
	if a:keyname ==# 'ComeFirst'
		...
		let a:setting.comeLast = xpt#util#RemoveDuplicateFrom(a:setting.comeLast, a:setting.comeFirst)
	elseif a:keyname ==# 'ComeLast'
		let a:setting.comeLast = xpt#util#SplitWith( a:value, ' ' )
		let a:setting.comeFirst = xpt#util#RemoveDuplicateFrom(a:setting.comeFirst, a:setting.comeLast)
	elseif a:keyname ==# 'postQuoter'
		let a:setting.postQuoter = a:value
	elseif a:keyname =~ '\V\^$'
		if a:value =~ '\V\^->keep<-'
			let a:setting.variables[a:keyname] = a:value[8:]
		elseif a:value =~ '\V\^->runr<-' || a:value =~ '\V\^->runp<-'
			let a:setting.variables[a:keyname] = a:value
		else
			let a:setting.variables[a:keyname] = xpt#eval#Eval(a:value, [b:xptemplateData.filetypes[b:xptemplateData.snipFileScope.filetype].funcs, a:setting.variables])
		endif
		let valuee = a:setting.variables[a:keyname]
		if a:keyname == "$_import_" && valuee[0:0] == "{"
			call g:MyMergeSSetting(a:setting, json_decode(valuee))
	...
	elseif a:keytype ==# 'post'
		...
		if type(a:value) == type('')
				let val = g:MyParseRepetition(b:xptemplateData.snipFileScope, a:setting.postFilters, a:value)
				let a:setting.postFilters[a:keyname] = xpt#flt#New(0, g:MyParseQuotedPostFilter(b:xptemplateData.snipFileScope, a:setting.postFilters, { 'end':'}}', 'start':'{{' }, val))
		endif
	else
		throw "unknown key name or type:" . a:keyname . ' ' . a:keytype
	endif
endfunction
fun! g:MyXSEThandler(setting, str)
	let start = 0
	let snippetLines = []
	let lines = (type(a:str) == type("")) ? split(a:str,"\n") : a:str
	let len = len(lines)
	while start < len
		let command = matchstr( lines[ start ], '\V\^XSETm\?\ze\s' )
		if command != ''
			let [key,val,start] = s:GetXSETkeyAndValue(lines,start, a:setting)
			if key == ''
				let start += 1
				continue
			endif
			let [keyname,keytype] = xpt#parser#GetKeyType(key)
			call s:HandleXSETcommandOld(a:setting,command,keyname,keytype,val)
		elseif lines[start] =~# '^\\XSET' " escaped XSET or XSETm
			let snippetLines += [lines[start][1:]]
		else
			let snippetLines += [lines[start]]
