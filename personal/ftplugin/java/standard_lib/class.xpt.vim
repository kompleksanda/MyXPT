...
XSET $getClass Class=getClass\()
XSET $hashCode int=hashCode\()
XSET $equals boolean=equals\(`Object obj^)
XSET $clone Object Throws: CloneNotSupportedException - if the object's class does not support the Cloneable interface=clone\()
XSET $toString String=toString\()
XSET $notify Wakes up a single thread that is waiting on this object's monitor\. If any threads are waiting on this object, one of them is chosen to be awakened=notify\()
XSET $notifyAll Wakes up all threads that are waiting on this object's monitor=notifyAll\()
XSET $wait (timeout) Causes the current thread to wait until either another thread invokes the notify or a specified amount of time has elapsed=wait\(`long timeout^)
XSET $wait (timeout, nanos)=wait\(`long timeout^, `int nanos^)
XSET $wait Causes the current thread to wait until another thread invokes the notify=wait\()
XSET $finalize Called by the garbage collector on an object when garbage collection determines that there are no more references to the object=finalize\()

XPT objects alias=_chooseXSET
XSET $Objects\.equals (a, b) boolean true if the arguments are equal to each other and false otherwise=Objects.equals\(`Object a^, `Object b^)
XSET $Objects\.deepEquals (a,b) boolean true if the arguments are deeply equal to each other and false otherwise=Objects.deepEquals\(`Object a^, `Object b^)
XSET $Objects\.hashCode int EX: hashCode\(null)\=\=0=hashCode\(`Object o^)
XSET $Objects\.hash (object\.\.\.) int Generates a hash code for a sequence of input values=Objects.hash\(`Object ... values^)
XSET $Objects\.toString String =Objects.toString\(`Object o^)
XSET $Objects\.toString (o, default) String Returns toString if the first argument is not null else second argument otherwise=Objects.toString\(`Object o^, `String nullDefault^)
XSET $Objects\.compare int =Objects.compare\(`T a^, `T b^, `Comparator <? super T> c^)
XSET $Objects\.requireNonNull Returns: obj if not null Throws: NullPointerException - if obj is null=Objects.requireNonNull\(`T obj^)
XSET $Objects\.requireNonNull (obj, message) Checks that the specified object reference is not null and throws a customized NullPointerException if it is=Objects.requireNonNull\(`T obj^, `String message^)
XSET $Objects\.isNull boolean=Objects.isNull\(`Object obj^)
XSET $Objects\.nonNull boolean Ex: filter(Objects::nonNull)=Objects.nonNull\(`Object obj^)
XSET $Objects\.requireNonNull (obj, supplier) this method allows creation of the message to be deferred until after the null check is made=Objects.requireNonNull\(`T obj^, `Supplier < String > messageSupplier^)
...
XSET $System alias=_chooseXSET
XSET $System\.in InputStream in=System.in
XSET $System\.out PrintStreamout=System.out
XSET $System\.err PrintStream=System.err
XSET $System\.setIn Reassigns the "standard" input stream. First, if there is a security manager, its checkPermission method is called with a RuntimePermission("setIO") permission to see if it's ok to reassign the "standard" input stream=System.setIn\(`InputStream in^)
XSET $System\.setOut Reassigns the "standard" output stream=System.setOut\(`PrintStream out^)
XSET $System\.setErr Reassigns the "standard" error output stream=System.setErr\(`PrintStream err^)
XSET $System\.console Console Returns the unique Console object associated with the current Java virtual machine, if any otherwise null=System.console\()
XSET $System\.inheritedChannel Channel Returns the channel inherited from the entity that created this Java virtual machine else null=System.inheritedChannel\()
XSET $System\.setSecurityManager Sets the System security If there is a security manager already installed, this method first calls the security manager's checkPermission method=System.setSecurityManager\(`SecurityManager s^)
XSET $System\.getSecurityManager SecurityManager=System.getSecurityManager()~
XSET $System\.currentTimeMillis long Returns the current time in milliseconds=System.currentTimeMillis\()
XSET $System\.nanoTime long Returns the current value of the running Java Virtual Machine's high-resolution time source, in nanoseconds=System.nanoTime\()
XSET $System\.arraycopy Copies an array from the specified source array, beginning at the specified position, to the specified position of the destination array=System.arraycopy\(`Object src^, `int srcPos^, `Object dest^, `int destPos^, `int length^)
XSET $System\.identityHashCode (x) int =System.identityHashCode\(`Object x^)
XSET $System\.getProperties Properties get the system properties=System.getProperties\()
XSET $System\.lineSeparator String the system-dependent line separator string=System.lineSeparator\()
XSET $System\.setProperties=System.setProperties\(`Properties props^)
XSET $System\.getProperty String Gets the system property indicated by the specified key=System.getProperty\(`String key^)
XSET $System\.getProperty (key, default) String=System.getProperty\(`String key^, `String def^)
XSET $System\.setProperty String Returns: the previous value of the system property=System.setProperty\(`String key^, `String value^)
XSET $System\.clearProperty String Removes the system property indicated by the specified key, Returns: the previous string value of the system property=System.clearProperty\(`String key^)
XSET $System\.getenv String Gets the value of the specified environment variable=System.getenv\(`String name^)
XSET $System\.getenv Map < String , String > Returns an unmodifiable string map view of the current system environment=System.getenv\()
XSET $System\.exit void Terminates the currently running Java Virtual Machine, The argument serves as a status code=System.exit\(`int status^)
XSET $System\.gc Runs the garbage collector=System.gc\()
XSET $System\.runFinalization Runs the finalization methods of any objects pending finalization=System.runFinalization\()
XSET $System\.load Loads the native library specified by the filename argument, The filename argument must be an absolute path name=System.load\(`String filename^)
XSET $System\.loadLibrary Loads the native library specified by the libname argument, The libname argument must not contain any platform specific prefix, file extension or path=System.loadLibrary\(`String libname^)
XSET $System\.mapLibraryName String Maps a library name into a platform-specific string representing a native library=System.mapLibraryName\(`String libname^)
